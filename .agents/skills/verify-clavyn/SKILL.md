...
docs live in `scripts/verify/` (agent-agnostic). This skill is the standard
Agent Skills entry point; agent-specific pointers (Devin, Claude Code, Cursor)
reference this same canonical CLI.
...
- The harness auto-starts the Vite dev server if it isn't running; set
  `CLAVYN_AUTO_START_DEV=0` to disable that.
- To drive a real `tauri dev`/packaged webview instead of the mocked
  renderer, set `CLAVYN_URL` and `CHROMIUM_EXECUTABLE_PATH` to point at its
  webview endpoint.
...
- Always run `doctor` first if anything seems off.
